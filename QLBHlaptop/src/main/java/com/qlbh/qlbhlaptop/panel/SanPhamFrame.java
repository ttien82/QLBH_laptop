...
/*
    *** Goi main() de goi frame ***
    SanPhamFrame.main(args);
*/

public class SanPhamFrame extends javax.swing.JFrame {
    
    private static final java.util.logging.Logger logger = java.util.logging.Logger.getLogger(SanPhamFrame.class.getName());
    ...
    private void InitTblSanPham()
    {
        ...
        model = new DefaultTableModel(columns, 0)
        {
            ...
            public Class<?> getColumnClass(int columnIndex) {
                if (columnIndex == 10) return ImageIcon.class; // Cột ảnh
                return String.class;
            }
        };
        //7.setRowHeight de HinhAnh co the duoc hien thi
    ...
    private void fillTableData() { 
        ...
        //3.Xu ly chuoi => row chua mang cac san pham
        String[] rows = data.split(";");

        for (String row : rows) {
            //4.Xu ly chuoi => tach tung row Sanpham data thanh cac Field
            row = row.replace("{", "").replace("}", "").trim();
            String[] fields = row.trim().split("\\s*,\\s*");

            //5.Lay anh tu thu muc images(cung cap voi 'src')
            String imagePath = "images/" + fields[10].trim(); //image column
            ImageIcon icon = null;
            ...
            model.addRow(new Object[]{
                fields[0].trim(), // MaSP
                fields[1].trim(), // TenSP
                fields[2].trim(), // TenNCC
                fields[3].trim(), // TenLoaiSP
                fields[4].trim(), // CPU
                fields[5].trim(), // Ram
                fields[6].trim(), // OCung
                fields[7].trim(), // CardManHinh
                fields[8].trim(), // GiaBan
                fields[9].trim(), // SoLuongTon
                icon              // HinhAnh
            });
        }
    ...
    private void initComponents() {
        ...
        tblSanPham.setCellSelectionEnabled(true);
    ...
    private void btnAddActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAddActionPerformed
        ...
        SanPham_Dialog_Them dialog = new SanPham_Dialog_Them(this); // this = SanPhamFrame
    ...
    private void btnEditActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnEditActionPerformed
        ...
        SanPham_Dialog_Sua dialog = new SanPham_Dialog_Sua(this, tblSanPham.getValueAt(selectedRow, 0).toString()); // this = SanPhamFrame
        dialog.addWindowListener(new java.awt.event.WindowAdapter() {
            @Override
            public void windowClosed(java.awt.event.WindowEvent e) {
                // Gọi hàm fill lại dữ liệu khi ThemSPFrame đóng
    ...
    private void btnDeleteActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnDeleteActionPerformed
        int result = JOptionPane.showConfirmDialog(
            ...
            "Bạn có chắc chắn muốn xóa sản phẩm này?",  // Nội dung
            "Xác nhận",                     // Tiêu đề
    ...
    public static void main(String args[]) {
        ...
        java.awt.EventQueue.invokeLater(() -> new SanPhamFrame().setVisible(true));
